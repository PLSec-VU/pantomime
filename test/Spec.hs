...
setupPlugin = do
  ...
  setSessionDynFlags $ updOptLevel 1 dflags
    { pluginModNames = mkModuleName "Pantomime" : pluginModNames dflags
...
progPaths = do
  let listDirectory' dir = do
        ...
        pure $ fmap (dir <>) files
  paths <- listDirectory' "tests/"
  let isSrc path = isSuffixOf ".hs" path && not ("ForceEmbedding.hs" `isSuffixOf` path)
