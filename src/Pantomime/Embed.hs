...
embed' subst sty repr = case sty of
  ...
  SArrayTy _ _ -> do
    ...
    let co' = mkSymCo $ mkSubCo co
    mkCast lit co'
  SPrimitiveTy _ -> throwE "embed': cannot directly embed a PrimitiveTy — use project' first"
  SLambda aty rty -> do
  ...
  SLEqTy _ _ -> throwE "embed': SLEqTy cannot be embedded directly"
  SKnownNatTy _n -> throwE "embed': SKnownNatTy cannot be embedded directly"
  ...
  SUnsafeEqualityTy {} -> do
    ...
    ty <- liftEff $ embedSTy subst sty
    (tc, args) <- failWithE "embed': expected TyCon application for UnsafeEqualityTy" $ splitTyConApp_maybe ty

    -- Fetch the 'UnsafeRefl' DataCon.
    dc <- case tyConDataCons_maybe tc of
      Just [dc] -> pure dc
      _ -> throwE "embed': expected a single data constructor for UnsafeEqualityTy"
    ...
    (kind, tyL, tyR) <- case args of
      ...
      _ -> throwE "embed': expected exactly three type arguments for UnsafeEqualityTy"
...
project' subst sty expr = case sty of
  SBoolTy -> hoistEff expr >>= \case
    Lit (Bool b) -> pure b
    _ -> throwE "project': expected a Bool literal"
  SIntegerTy -> hoistEff expr >>= \case
    Lit (Integer i) -> pure i
    _ -> throwE "project': expected an Integer literal"
  SBitVecTy _n -> do
    Reduction co _ <- liftEff $ normaliseSTy subst Nominal sty
    inner <- hoistEff expr
    ...
    case expr' of
      ...
      _ -> throwE "project': expected a BitVec literal"
  SArrayTy _ _ -> do
    Reduction co _ <- liftEff $ normaliseSTy subst Nominal sty
    inner <- hoistEff expr
    expr' <- mkCast inner $ mkSubCo co
    case expr' of
      ...
      _ -> throwE "project': expected an Array literal"
  SPrimitiveTy pty -> do
    _ <- hoistEff expr
  ...
  SNaturalTy -> throwE "project': SNaturalTy cannot be projected directly"
  SNatural -> throwE "project': SNatural cannot be projected directly"
  SAddTy _ _ -> throwE "project': SAddTy cannot be projected directly"
  SLEqTy _ _ -> do
  ...
  SKnownNatTy _ -> do
    ...
    co <- failWithE "project': could not construct KnownNat to Integer coercion" do
    ...
    expr' >>= \case
      ...
      _ -> throwE "project': expected a concrete Integer for KnownNat projection"
  STYPE _ -> throwE "project': STYPE cannot be projected directly"
  SRuntimeRepTy -> throwE "project': SRuntimeRepTy cannot be projected directly"
  SBoxedRep _ -> throwE "project': SBoxedRep cannot be projected directly"
  SLevityTy -> throwE "project': SLevityTy cannot be projected directly"
  SLifted -> throwE "project': SLifted cannot be projected directly"
  SUnsafeEqualityTy {} -> do
    expr' <- hoistEff expr
    let (_spine, args) = collectArgs expr'
    case args of
      [_kind, _ty, co] -> liftEff $ forceCo co
      _ -> throwE "project': expected exactly three arguments for UnsafeEqualityTy"
